...
Motion::Motion() : Process<Motion>(true) {
...
void Motion::tick() {
  ...
  auto cfg_client = MotionConfigClient::getinstance(CFG_SERVER_FILE,
                                                Endpoint::CFG_PUB,
                                                Endpoint::CFG_REPLY);
  GaitStateManager manager;

  mynode.nonblock_sub(Endpoint::BEHAVIOUR, "ActionCommand", [&](string data) {
    behave_req = ActionCommand::All::restore(data);
  });
