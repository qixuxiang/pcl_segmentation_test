#include "HumanRobot.hpp"
#include <fstream>
#include "motion/MotionConfig.hpp"
#include "misc/utils/logger/logger.hpp"
#include "motion/GaitStateLib/GaitStateBase.hpp"
#include "motion/GaitStateManager.hpp"
#include "motion/GaitStateSupportLib/SplineThree.hpp"
#include "onedCtrl.hpp"

// TODO: tons of memory leak...
...
HumanRobot::HumanRobot(transitHub *port, RobotStatus *rs,
  ...
  LOG(INFO, "        [ Constructing HumanRobot" );
  readOptions();
...
void HumanRobot::readOptions() {
  ...
  LOG(TRANSITHUB_DEBUG, "            HumanRobot read options succeed )" );
}
...
boost_matrix HumanRobot::roty(double pit) {
  boost_matrix Ry(3, 3);
  Ry(0, 0) = cos(pit);
  Ry(0, 1) = 0;
  Ry(0, 2) = sin(pit);
...
boost_matrix HumanRobot::rotz(double yaw) {
  boost_matrix Rz(3, 3);
  Rz(0, 0) = cos(yaw);
  Rz(0, 1) = -sin(yaw);
  Rz(0, 2) = 0;
...
boost_matrix HumanRobot::rot2Tx(double roll, double x, double y, double z) {
  boost_matrix Rx(4, 4);
  Rx(0, 0) = 1;
  Rx(0, 1) = 0;
  Rx(0, 2) = 0;
...
boost_matrix HumanRobot::rot2Ty(double pit, double x, double y, double z) {
  boost_matrix Ry(4, 4);
  Ry(0, 0) = cos(pit);
  Ry(0, 1) = 0;
  Ry(0, 2) = sin(pit);
...
boost_matrix HumanRobot::rot2Tz(double yaw, double x, double y, double z) {
  boost_matrix Rz(4, 4);
  Rz(0, 0) = cos(yaw);
  Rz(0, 1) = -sin(yaw);
  Rz(0, 2) = 0;
...
boost_matrix HumanRobot::rpy2t(double roll, double pit, double yaw, double x,
                               double y, double z) {
...
boost_matrix HumanRobot::rpy2r(double roll, double pit,
                               double yaw)  /// ypr order in gyroscope
{
  //旋转的顺序是 yaw , pitch , roll
 ...
 *
 ***********************************************/
boost_matrix HumanRobot::Array2row(double *t_array, int row) {
 ...
 *详细可见 《仿人机器人第》
 ***********************************************/
double *HumanRobot::get_Angle(const double *tChest, const double *tAnkle,
  ...
  if (m_robotCtrl.supportStatus == DOUBLE_BASED) {
    if (whleg == 0)  // 0 means rightLeg
      lby = -RobotPara::hip_distance / 2 -
            RobotPara::yzmp;  //理解时可以先将yzmp 假想成0
    else
  ...
  q[2 - 1] = -atan2(r(0, 0), (r(2, 0) / abs(r(2, 0))) *
                                 sqrt(r(1, 0) * r(1, 0) + r(2, 0) * r(2, 0))) +
             q6a;
  ...
  q[5 - 1] = atan2(RR(2, 1), -RR(0, 1) * sin(q[6 - 1]) +
                                 RR(1, 1) * cos(q[6 - 1]));  // 髋关节rol角x
  ...
  theta3 =
      acos((z * z + x * x + l3 * l3 - l4 * l4) / 2 / l3 / sqrt(x * x + z * z));
 ...
 *
 ***********************************************/
double *HumanRobot::curveCreate(double start, double mid, double last,
                                int num) {
 ...
 *
 ***********************************************/
void HumanRobot::data2q(const int data[], double lq[], double rq[]) {
  ...
  lq[6] =
      data[8 - 1] / m_motor_k[8 - 1] * 1.0 / m_motor_zf[8 - 1];  // id 8,15,16
  ...
  rq[6] =
      data[1 - 1] / m_motor_k[1 - 1] * 1.0 / m_motor_zf[1 - 1];  // id 1,17,18
 ...
 *
 ***********************************************/
void HumanRobot::q2data(int data[], const double lq[], const double rq[]) {
  ...
  data[17 - 1] =
      (int)round(rq[7] * m_motor_k[17 - 1] * 1.0 * m_motor_zf[17 - 1]);
  data[18 - 1] =
      (int)round(rq[8] * m_motor_k[18 - 1] * 1.0 * m_motor_zf[18 - 1]);

  data[14 - 1] =
      (int)round(lq[0] * m_motor_k[14 - 1] * 1.0 * m_motor_zf[14 - 1]);
  data[13 - 1] =
      (int)round(lq[1] * m_motor_k[13 - 1] * 1.0 * m_motor_zf[13 - 1]);
  data[12 - 1] =
      (int)round(lq[2] * m_motor_k[12 - 1] * 1.0 * m_motor_zf[12 - 1]);
  data[9 - 1] = (int)round(lq[3] * m_motor_k[9 - 1] * 1.0 * m_motor_zf[9 - 1]);
  data[10 - 1] =
      (int)round(lq[4] * m_motor_k[10 - 1] * 1.0 * m_motor_zf[10 - 1]);
  data[11 - 1] =
      (int)round(lq[5] * m_motor_k[11 - 1] * 1.0 * m_motor_zf[11 - 1]);
  data[8 - 1] = (int)round(lq[6] * m_motor_k[8 - 1] * 1.0 * m_motor_zf[8 - 1]);
  data[15 - 1] =
      (int)round(lq[7] * m_motor_k[15 - 1] * 1.0 * m_motor_zf[15 - 1]);
  data[16 - 1] =
      (int)round(lq[8] * m_motor_k[16 - 1] * 1.0 * m_motor_zf[16 - 1]);
}
 ...
 *
 ************************************************/
void HumanRobot::Leg_up(double rsx, double rsy, double rst) {
  ...
  for (int n = 0; n < m; n++) {
    if (n == 0) {
      px_c[n] = -a * (C - 1) / D * (x_d[n] - C * x_f0 - Tc * S * vxy[0]) -
                b * S / Tc / D * (vx_d[n] - S / Tc * x_f0 - C * vxy[0]);
      x_f[0] = C * x_f0 + Tc * S * vxy[0] + (1 - C) * px_c[0];
      vx_f[0] = S / Tc * x_f0 + C * vxy[0] - S / Tc * px_c[0];
    } else {
      px_c[n] =
          -a * (C - 1) / D * (x_d[n] - C * x_f[n - 1] - Tc * S * vx_f[n - 1]) -
          b * S / Tc / D * (vx_d[n] - S / Tc * x_f[n - 1] - C * vx_f[n - 1]);
  ...
  for (int n = 0; n < m; n++) {
    if (n == 0) {
      py_c[n] = -a * (C - 1) / D * (y_d[n] - C * y_f0 - Tc * S * vxy[1]) -
                b * S / Tc / D * (vy_d[n] - S / Tc * y_f0 - C * vxy[1]);
      y_f[0] = C * y_f0 + Tc * S * vxy[1] + (1 - C) * py_c[0];
      vy_f[0] = S / Tc * y_f0 + C * vxy[1] - S / Tc * py_c[0];
    } else {
      py_c[n] =
          -a * (C - 1) / D * (y_d[n] - C * y_f[n - 1] - Tc * S * vy_f[n - 1]) -
          b * S / Tc / D * (vy_d[n] - S / Tc * y_f[n - 1] - C * vy_f[n - 1]);
  ...
  for (int i = 0; i < m; i++) {
    if (i == 0) {
      for (double t = RobotPara::dt; t < Tsup + RobotPara::dt;
           t = t + RobotPara::dt) {
        xt[num] = (x_f0 - px_c[i]) * cosh(t / Tc) + Tc * vxy[0] * sinh(t / Tc) +
                  px_c[i];
        num++;
      }
    } else {
      for (double t = RobotPara::dt; t < Tsup + RobotPara::dt;
           t = t + RobotPara::dt) {
        xt[num] = (x_f[i - 1] - px_c[i]) * cosh(t / Tc) +
                  Tc * vx_f[i - 1] * sinh(t / Tc) + px_c[i];
        num++;
      }
    }
  }
  ...
  for (int i = 0; i < m; i++) {
    if (i == 0) {
      for (double t = RobotPara::dt; t < Tsup + RobotPara::dt;
           t = t + RobotPara::dt) {
        yt[num] = (y_f0 - py_c[i]) * cosh(t / Tc) + Tc * vxy[1] * sinh(t / Tc) +
                  py_c[i];
        num++;
      }
    } else {
      for (double t = RobotPara::dt; t < Tsup + RobotPara::dt;
           t = t + RobotPara::dt) {
        yt[num] = (y_f[i - 1] - py_c[i]) * cosh(t / Tc) +
                  Tc * vy_f[i - 1] * sinh(t / Tc) + py_c[i];
        num++;
      }
    }
  }
  ...
  if (m_robotCtrl.supportStatus == LEFT_BASED) {
    ...
    if (tst >= 0)
      targetCtrl.ra[1] =
          py_c[1] + RobotPara::Ankle_dev_r +
          tst * abs(tsx) / RobotPara::mid_x_max * RobotPara::Ankle_dev_r_tk;
    else
    ...
    r_s = curveCreate(m_robotCtrl.ra[2], m_AnkleH_mid_r, m_AnkleH_last_r,
                      RobotPara::stepnum);
    ...
    if (tst <= 0)
      targetCtrl.la[1] =
          py_c[1] + RobotPara::Ankle_dev_l -
          tst * abs(tsx) / RobotPara::mid_x_max * RobotPara::Ankle_dev_l_tk;
    else
    ...
    l_s = curveCreate(m_robotCtrl.la[2], m_AnkleH_mid_l, m_AnkleH_last_l,
                      RobotPara::stepnum);
  ...
  if (m_robotCtrl.supportStatus == LEFT_BASED) {
    ...
    LOG(ERROR,
        "HumanRobot::Leg_up robotCtrl.supportStatus not right or left" );
  }
  cm_residul = x_amend;
}

double HumanRobot::getThetaAmend(const double tsx) {
  double k, theta_amd;
  if (tsx <= 0) {
    k = (RobotPara::step_theta_amend - RobotPara::back_theta_amend) /
        (0 - RobotPara::back_x_max);
    theta_amd = RobotPara::back_theta_amend + k * (tsx - RobotPara::back_x_max);
  } else if (tsx <= RobotPara::mid_x_max) {
    k = (RobotPara::mid_theta_amend - RobotPara::step_theta_amend) /
        (RobotPara::mid_x_max - 0);
    theta_amd = RobotPara::step_theta_amend + k * (tsx - 0);
  } else {
    if (abs(RobotPara::top_x_max - RobotPara::mid_x_max) < epso)
      k = 0;
    else
      k = (RobotPara::top_theta_amend - RobotPara::mid_theta_amend) /
          (RobotPara::top_x_max - RobotPara::mid_x_max);
...
double HumanRobot::getRightAnkleMidHigh(const double tsx) {
  double k, ah;
  if (abs(tsx) <= RobotPara::mid_x_max) {
    k = (RobotPara::ah_mr_mid - RobotPara::ah_mr_zero) /
        (RobotPara::mid_x_max - 0);
...
double HumanRobot::getLeftAnkleMidHigh(const double tsx) {
  double k, ah;
  if (abs(tsx) <= RobotPara::mid_x_max) {
    k = (RobotPara::ah_ml_mid - RobotPara::ah_ml_zero) /
        (RobotPara::mid_x_max - 0);
    ...
    if (abs(RobotPara::top_x_max - RobotPara::mid_x_max) < epso)
      ...
      k = (RobotPara::ah_ml_top - RobotPara::ah_ml_mid) /
          (RobotPara::top_x_max - RobotPara::mid_x_max);
 ...
 *
 ***********************************************/
void HumanRobot::getVxyf0(double tsx, double vxy[]) {
  vector<double> sx, sy, st;
  sx.push_back(m_robotCtrl.robot_x);
  ...
  for (int i = 0; i < m; i++) {
    if (i == 0) {
      ...
      px[i] =
          px[i - 1] + cos(st[i]) * sx[i] + sin(st[i]) * pow(-1, i + 1) * sy[i];
      py[i] =
          py[i - 1] - cos(st[i]) * pow(-1, i + 1) * sy[i] + sin(st[i]) * sx[i];
    }
  }
  /// walk para
  // double cx[m], cy[m];
...
void HumanRobot::runWalk(double tsx, double tsy, double tst) {
  ...
  if (true) {  // do smooth in real , can also do in command
    ...
    if (abs(tsx - tmp_xyt.robot_x) >= 2.5 &&
        *m_manager->gaitState != WALKRIGHTKICK &&
        *m_manager->gaitState != WALKLEFTKICK) {
 ...
 *
 ***********************************************/
double HumanRobot::getTheta(double v) {
  if (v >= 4)
    return 5;
  else if (v < 4 && v > 2)
    return 10;
 ...
 *
 ***********************************************/
void HumanRobot::getAngle_serial(const RobotCtrl targetCtrl, int dataArray[],
                                 const bool isExcute) {
  double hip_target[6];
  for (int i = 0; i < 6; i++) {
  ...
  double cm[6], ra[6], la[6], rh[6], lh[6], H_cm[6], H_ra[6], H_la[6], H_rh[6],
      H_lh[6];
  // onedCtrl 为
  ...
  if (isExcute) {
    ...
    lhx_target[1] = 0;
    ankleCtrl.oned_analysis(lhx_out, lhx_start, lhx_target,
                            m_robotCtrl.num_left);
    ...
    lhz_target[1] = 0;
    ankleCtrl.oned_analysis(lhz_out, lhz_start, lhz_target,
                            m_robotCtrl.num_left);
    ...
    lhroll_target[1] = 0;
    rpyCtrl.oned_analysis(lhroll_out, lhroll_start, lhroll_target,
                          m_robotCtrl.num_left);
    ...
    lhpitch_target[1] = 0;
    rpyCtrl.oned_analysis(lhpitch_out, lhpitch_start, lhpitch_target,
                          m_robotCtrl.num_left);
    ...
    lhyaw_target[1] = 0;
    rpyCtrl.oned_analysis(lhyaw_out, lhyaw_start, lhyaw_target,
                          m_robotCtrl.num_left);
    /// lh update
    lh[0] = lhx_out[0];
    lh[1] = lhy_out[0];
    ...
    rhx_target[1] = 0;
    ankleCtrl.oned_analysis(rhx_out, rhx_start, rhx_target,
                            m_robotCtrl.num_left);
    ...
    rhz_target[1] = 0;
    ankleCtrl.oned_analysis(rhz_out, rhz_start, rhz_target,
                            m_robotCtrl.num_left);
    ...
    rhroll_target[1] = 0;
    rpyCtrl.oned_analysis(rhroll_out, rhroll_start, rhroll_target,
                          m_robotCtrl.num_left);
    ...
    rhpitch_target[1] = 0;
    rpyCtrl.oned_analysis(rhpitch_out, rhpitch_start, rhpitch_target,
                          m_robotCtrl.num_left);
    ...
    rhyaw_target[1] = 0;
    rpyCtrl.oned_analysis(rhyaw_out, rhyaw_start, rhyaw_target,
                          m_robotCtrl.num_left);
    /// rh update
    rh[0] = rhx_out[0];
    rh[1] = rhy_out[0];
    ...
    laz_target[1] = 0;
    ankleCtrl.oned_analysis(laz_out, laz_start, laz_target,
                            m_robotCtrl.num_left);
    ...
    laroll_target[1] = 0;
    rpyCtrl.oned_analysis(laroll_out, laroll_start, laroll_target,
                          m_robotCtrl.num_left);
    ...
    lapitch_target[1] = 0;
    rpyCtrl.oned_analysis(lapitch_out, lapitch_start, lapitch_target,
                          m_robotCtrl.num_left);
    ...
    layaw_target[1] = 0;
    rpyCtrl.oned_analysis(layaw_out, layaw_start, layaw_target,
                          m_robotCtrl.num_left);
    /// la update
    ...
    raz_target[1] = 0;
    ankleCtrl.oned_analysis(raz_out, raz_start, raz_target,
                            m_robotCtrl.num_left);
    ...
    raroll_target[1] = 0;
    rpyCtrl.oned_analysis(raroll_out, raroll_start, raroll_target,
                          m_robotCtrl.num_left);
    ...
    rapitch_target[1] = 0;
    rpyCtrl.oned_analysis(rapitch_out, rapitch_start, rapitch_target,
                          m_robotCtrl.num_left);
    ...
    rayaw_target[1] = 0;
    rpyCtrl.oned_analysis(rayaw_out, rayaw_start, rayaw_target,
                          m_robotCtrl.num_left);
    /// ra update
    ...
    cmx_target[1] = 0;
    ankleCtrl.oned_analysis(cmx_out, cmx_start, cmx_target,
                            m_robotCtrl.num_left);
    ...
    cmy_target[1] = 0;
    ankleCtrl.oned_analysis(cmy_out, cmy_start, cmy_target,
                            m_robotCtrl.num_left);
    ...
    cmz_target[1] = 0;
    ankleCtrl.oned_analysis(cmz_out, cmz_start, cmz_target,
                            m_robotCtrl.num_left);
    ...
    cmroll_target[1] = 0;
    rpyCtrl.oned_analysis(cmroll_out, cmroll_start, cmroll_target,
                          m_robotCtrl.num_left);
    ...
    cmpitch_target[1] = 0;
    rpyCtrl.oned_analysis(cmpitch_out, cmpitch_start, cmpitch_target,
                          m_robotCtrl.num_left);
    ...
    cmyaw_target[1] = 0;
    rpyCtrl.oned_analysis(cmyaw_out, cmyaw_start, cmyaw_target,
                          m_robotCtrl.num_left);
    /// cm update
    ...
    if (m_robotCtrl.num_left == 1 &&
        m_robotCtrl.supportStatus != DOUBLE_BASED)  // bug
    {
      ...
      if (m_robotCtrl.supportStatus == LEFT_BASED) {
        ...
        S_la[0] =
            (c1 * (la[0]) - (c1 * cm[0] + cm[1] * s1) + (s1 * (la[1]))) - a_tmp;
        ...
        S_ra[0] =
            (c1 * (ra[0]) - (c1 * cm[0] + cm[1] * s1) + (s1 * (ra[1]))) - a_tmp;
        S_ra[1] = (c1 * (ra[1]) - (c1 * cm[1] - cm[0] * s1) - (s1 * (ra[0])));
        S_la[0] = 0;
        S_la[1] = (c1 * (la[1]) - (c1 * cm[1] - cm[0] * s1) - (s1 * (la[0])));
        S_layaw = 0;
    ...
    cmdy_target[1] = 0;
    rpyCtrl.oned_analysis(cmdy_out, cmdy_start, cmdy_target,
                          m_robotCtrl.num_left);
...
void HumanRobot::dofirstStep() {
  ...
  double ankle_distance_big =
      RobotPara::ankle_distance + 2 * RobotPara::yzmp + RobotPara::Ankle_dev_l -
      RobotPara::Ankle_dev_r;  //机器人走路时候即最大情况下的两脚间距
  double ankle_distance_small =
      ori.la[1] - ori.ra[1];  //机器人最开始的两脚间距，即髋关节的间距
  ...
  double cm_excute =
      getPieceWise(x_cm, y_cm, m_robotCtrl.la[1] - m_robotCtrl.ra[1]);
  ...
  for (int i = 0; i < firstStep_method;
       i++)  // template use for firststep stepnum
    runWalk(0, 0, 0);
  // runWalk(0, 0, 0.5);
  // runWalk(0, 0, 0.5);
...
void HumanRobot::doCrouchFromStandMotor(const int stepnum_) {
  // cout<<"motor"<<endl;
...
void HumanRobot::doCrouchFromStand(const int stepnum_) {  //
  RobotCtrl targetCtrl(m_robotCtrl);
  m_robotCtrl.setAutoMode();
  // ankle
  if (*m_manager->gaitState == SETUPFRONTDOWN ||
      *m_manager->gaitState == SETUPBACKDOWN ||
      *m_manager->gaitState == SETUPLEFTDOWN ||
      *m_manager->gaitState == LEFTKICK || *m_manager->gaitState == KICK ||
      *m_manager->gaitState == SETUPRIGHTDOWN) {
    targetCtrl.la[0] = 0;
...
void HumanRobot::staticExit() {
  if (*m_manager->goal_gaitState == STANDUP ||
      *m_manager->goal_gaitState == KICK ||
      *m_manager->goal_gaitState == GOALIEMID ||
      *m_manager->goal_gaitState == LEFTKICK ||
      *m_manager->goal_gaitState == CROUCH ||
      *m_manager->goal_gaitState == GOALIELEFT ||
      *m_manager->goal_gaitState == GOALIERIGHT ||
      *m_manager->goal_gaitState == SETUPBACKDOWN ||
      *m_manager->goal_gaitState == SETUPFRONTDOWN ||
      *m_manager->goal_gaitState == SETUPLEFTDOWN ||
      *m_manager->goal_gaitState == SETUPRIGHTDOWN) {
    LOG(TRANSITHUB_DEBUG, "[Crouch] exit() do nothing. " );
    ...
    LOG(TRANSITHUB_DEBUG, "[Crouch] exit() do firststep." );
  }
