...
void onedCtrl::oned_analysis(double x_out[], const double x_start[],
  ...
  // S2
  S2 = (vmax * vmax - vxf * vxf) / 2 / amax +
       (vmax * vmax - vx0 * vx0) / 2 / amax;
  ...
  amax_calculate =
      (-tmp_b + sqrt(tmp_b * tmp_b - 4 * tmp_a * tmp_c)) / 2 / tmp_a;
  ...
  if (cake == 1) {
    ...
    if (dT <= t1) {
      ...
      tmp_out[0] = (vx0 + vmax) * t1 / 2 + (t2)*vmax +
                   (vmax + tmp_out[1]) * (dT - t1 - t2) / 2;
    }
  }
  // case 2
  if (cake == 2) {
