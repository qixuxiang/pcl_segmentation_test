...
inline void VecPos::operator=(const VecPos& p) {
  m_x = p.m_x;
  m_y = p.m_y;
}
...
inline VecPos VecPos::operator+(const VecPos& p) const {
  VecPos temp(0, 0);
  temp.m_x = m_x + p.m_x;
  temp.m_y = m_y + p.m_y;
  return temp;
...
inline bool VecPos::operator==(const VecPos& p) const {
...
inline void VecPos::rotate(double angle) {
  double x = m_x;
  double y = m_y;
