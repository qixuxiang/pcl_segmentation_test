...
GaitStateKick::GaitStateKick(I_HumanRobot *robot, GaitStateManager *manager)
    : GaitStateBase(2, KICK, robot), manager(manager) {
  loadGaitFile();
}
...
GaitStateKick::~GaitStateKick() {
}

void GaitStateKick::entry() {
  // if(m_right_kick)
  //   rightKickEntry();
  // else
  //   leftKickEntry();
  ...
  // cout<<robot->m_hipHeight<<" "<<robot->m_cm_dx<<endl;
    ...
    for (int j = 0; j < step; j++) {
      ...
      if (i == 2 && j == 0) {
        if (m_right_kick) {
          ...
          dataSmall = robot->curveCreate(
              dataArray[5 - 1], 0,
              dataArray[5 - 1] + (dataR_[14][i] / 360 * 4096), step / 2);
        } else {
          dataBig = robot->curveCreate(
              dataArray[9 - 1], 0,
              dataArray[9 - 1] + (dataL_[15][i] / 360 * 4096), step);
          dataSmall = robot->curveCreate(
              dataArray[12 - 1], 0,
              dataArray[12 - 1] + (dataL_[14][i] / 360 * 4096), step / 2);
        }
      }
      if (i == 3 && j == 0) {
        if (m_right_kick) {
          ...
          dataSmallb =
              robot->curveCreate(dataL_[14][i] / 360 * 4096, 0, 0, step / 2);
        }
      }
      if (i == 2) {
        if (m_right_kick) {
          ...
          dataArray[6 - 1] =
              dataArray[6 - 1] + int(dataR_[15][i] * j / step / 360 * 4096);
        } else {
          dataArray[9 - 1] = dataBig[j];  // dataArray[9-1] +
                                          // int(dataL_[15][i]*j/step/360*4096);
          dataArray[13 - 1] =
              dataArray[13 - 1] + int(dataL_[15][i] * j / step / 360 * 4096);
        }
      }
      if (i == 2 && j > step / 2) {
        if (m_right_kick)
      ...
      if (i == 3) {
        if (m_right_kick) {
          ...
          dataArray[6 - 1] = dataArray[6 - 1] +
                             ...
                             int(dataR_[15][i] * j / step / 360 * 4096);
        } else {
          dataArray[9 - 1] = dataArray[9 - 1] + dataBigb[j];
          dataArray[13 - 1] = dataArray[13 - 1] +
                              int(dataL_[15][i] / 360 * 4096) -
                              int(dataL_[15][i] * j / step / 360 * 4096);
        }
...
void GaitStateKick::execute() {
  // robot->doCrouchFromStand(30);
}

void GaitStateKick::exit() {
  if (crouchBool_)
