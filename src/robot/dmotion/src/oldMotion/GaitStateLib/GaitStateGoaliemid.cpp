...
GaitStateGoaliemid::GaitStateGoaliemid(I_HumanRobot* robot)
    : GaitStateBase(2, GOALIEMID, robot) {
}

GaitStateGoaliemid::~GaitStateGoaliemid() {
}

// void GaitStateGoaliemid::readOptions(
...
void GaitStateGoaliemid::doLiftbothHand() {
  ...
  for (int i = 0; i < m_stepnum; i++) {
    ...
    dataArray[15 - 1] =
        dataArray[15 - 1] + m_zf_15 * i * 1.0 / m_stepnum * 45.0 * 4096 / 360;
    dataArray[17 - 1] =
        dataArray[17 - 1] + m_zf_17 * i * 1.0 / m_stepnum * 45.0 * 4096 / 360;
    robot->doTxTask(dataArray);
  }
}

void GaitStateGoaliemid::doGoalieMid() {
  RobotCtrl targetCtrl = robot->m_robotCtrl;
  ...
  for (int i = 0; i < m_stepnum; i++) {
    ...
    robot->getAngle_serial(targetCtrl, dataArray, 1);
    dataArray[15 - 1] =
        dataArray[15 - 1] + m_zf_15 * i * 1.0 / m_stepnum * 20.0 * 4096 / 360;
    dataArray[17 - 1] =
        dataArray[17 - 1] + m_zf_17 * i * 1.0 / m_stepnum * 20.0 * 4096 / 360;
    robot->doTxTask(dataArray);
  }
}

void GaitStateGoaliemid::doRecover() {
