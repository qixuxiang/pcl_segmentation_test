...
static void
calcBoardCornerPositions(Size boardSize, float squareSize, vector<Point3f>& corners, CalibSettings::Pattern patternType)
{
    corners.clear();
...
static bool
runCalibration(CalibSettings& s,
               Size& imageSize,
               Mat& cameraMatrix,
               Mat& distCoeffs,
...
bool
runCalibrationAndSave(CalibSettings& s, Size imageSize, Mat& cameraMatrix, Mat& distCoeffs, vector<vector<Point2f>> imagePoints)
{
...
main()
{
    ...
    for (auto image_name : imagesName) {
        ...
        found = findChessboardCorners(view, s.getBoardSize(), pointBuf, CALIB_CB_ADAPTIVE_THRESH | CALIB_CB_FAST_CHECK | CALIB_CB_NORMALIZE_IMAGE);
        if (found) {
            Mat viewGray;
