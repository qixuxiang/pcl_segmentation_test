...
namespace dvision {
bool
IPM::initGetHomography(Point3d cameraLocation, Point3d cameraOrientation, Mat& homoFor, Mat& homoBack)
{
    ...
    // vector<Point2d> cornerUndistortImgPoint;
    // cornerUndistortImgPoint.push_back(Point2d(0, 0));
    // cornerUndistortImgPoint.push_back(Point2d(0, params.camera.heightUnDistortion - 1));
    // cornerUndistortImgPoint.push_back(Point2d(params.camera.widthUnDistortion - 1, 0));
    // cornerUndistortImgPoint.push_back(Point2d(params.camera.widthUnDistortion - 1, params.camera.heightUnDistortion - 1));
    //
    // vector<Point2d> cornerRealPoint;
...
IPM::calculatePoints(vector<Point2f>& contour, vector<Point2f>& resContour)
{
    ...
    //     double x = (contour[i].x - (params.camera.widthUnDistortion - params.camera.width) / 2 - params.camera.cx) / params.camera.fx;
    //     double y = (contour[i].y - (params.camera.heightUnDistortion - params.camera.height) / 2 - params.camera.cy) / params.camera.fy;
    //
